...
// CODE HERE
const backToOriginal = (event) => {
    event.target.classList.toggle("ogColors")
}
sayHelloButton.addEventListener("mouseout", backToOriginal)
...
const repeatMyParam = () => {
    ...
    axios.get(`http://localhost:3000/repeat/Messica`)
    .then(response => {
        console.log(response.data)
...
const sendAQuery = () => {
    axios.get(`http://localhost:3000/query-test?Mess`)
    .then(response => {
        console.log(response.data)
    })
}

queryButton.addEventListener("click", sendAQuery)
